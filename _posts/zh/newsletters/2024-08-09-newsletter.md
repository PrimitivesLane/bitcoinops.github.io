...
- **<!--faster-seed-exfiltration-attack-->更快的种子外泄攻击**：Lloyd Fournier、Nick Farrow 和 Robin Linus 宣布了 [Dark Skippy][]，这是一种改进的从比特币签名设备中窃取密钥的方法，他们此前已经[负责任地披露][topic responsible disclosures]给大约 15 个不同的硬件签名设备供应商。_密钥外泄_ 是指交易签名代码有意地制作成签名，以泄露关于底层密钥材料（例如私钥或 [BIP32 HD 钱包种子][topic bip32]）的信息。一旦攻击者获取到用户的种子，他们可以随时窃取用户的任何资金（包括在导致外泄的交易中所花费的资金，如果攻击者迅速采取行动的话）。
